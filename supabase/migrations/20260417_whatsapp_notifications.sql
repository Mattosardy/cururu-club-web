...
do $$
begin
    if to_regclass('public.socios') is not null
       and not exists (
           select 1
           from pg_constraint
           where conname = 'notificaciones_programadas_socio_id_fkey'
             and conrelid = 'public.notificaciones_programadas'::regclass
       ) then
        alter table public.notificaciones_programadas
            add constraint notificaciones_programadas_socio_id_fkey
