...
const runtimeConfig = window.CURURU_RUNTIME_CONFIG || {};
const runtimeSupabaseConfig = window.__SUPABASE_CONFIG__ || {};
const isLocalHost = ['127.0.0.1', 'localhost'].includes(window.location.hostname);

const SUPABASE_URL = runtimeSupabaseConfig.url || (
    isLocalHost
        ? runtimeConfig.localSupabaseUrl
        : runtimeConfig.supabaseUrl
);
const SUPABASE_ANON_KEY = runtimeSupabaseConfig.key || (
    isLocalHost
        ? runtimeConfig.localSupabasePublishableKey
...
function renderizarEstrellas(promedio, totalCalificaciones = 0) {
    ...
    if (totalCalificaciones > 0) {
        html += `<span>(${promedio.toFixed(1)})</span>`;
    }
    html += '</div>';
    return html;
}
