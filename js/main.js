...
function mostrarSeccion(seccionId) {
    const destino = mainSections.includes(seccionId) ? seccionId : 'inicio';
...
async function verificarSesion() {
    await actualizarUIporRol();
    await cargarContenidoInstitucional();
    await cargarNoticias();
    await cargarActividadesPublicas();
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    document.getElementById('formRegisterMagic')?.addEventListener('submit', async (event) => {
        ...
        const resultado = await loginConEmail(email);
