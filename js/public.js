        ...
        return `
            <div class="producto-card" data-producto='${JSON.stringify(producto).replace(/'/g, '&#39;')}'>
                ...
                <div style="padding:15px;text-align:left;">
                    ...
                    <div style="color:#a0b890;font-size:0.9rem;margin-bottom:10px;">${escapeHtml(indicaSativa)}</div>
                    <button class="btn-mas-info" onclick="event.stopPropagation();mostrarMasInfo('${producto.id}')" style="background:transparent;border:1px solid #7ca35a;color:#7ca35a;padding:8px 16px;border-radius:20px;cursor:pointer;width:100%;margin-bottom:10px;"><i class="fas fa-plus-circle"></i> Información</button>
                    <button class="btn-reservar-producto" onclick="event.stopPropagation();abrirModalDesdeBoton('${producto.id}')" style="background:#7ca35a;border:none;color:#0f190c;padding:10px;border-radius:25px;cursor:pointer;font-weight:bold;width:100%;" ${!disponible ? 'disabled' : ''}><i class="fas fa-calendar-check"></i> Reservar</button>
